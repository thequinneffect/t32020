...
int main() {
    ...
    for (int i=0; i < q; i++) {
        ...
        if (l == r) {
            ...
            propagation[l] += troy(1) % MODNUM;
            propagation[l+1] += (-b*troy(1) + MODNUM) % MODNUM; // and now prop[l+1] += a*prop[l-1] + b*prop[l] i.e. prop[l+1] += 0 + b*troy(1) i.e. net result of prop[l+1] is -b*troy(1)+b*troy(1) == 0
            propagation[l+2] += (-a*troy(1) + MODNUM) % MODNUM; // prop[l+2] += a*prop[l] + b*prop[l+1] = a*troy(1) + b*0 (from above) = a*troy(0) i.e. net result of prop[l+2] is -a*troy(1)+a*troy(1) == 0
            ...
            propagation[l] += troy(1) % MODNUM; // because (i-l+1) = 1 when i=l
            propagation[l+1] += (troy(2) -b*troy(1) + MODNUM) % MODNUM; // -b because prop[l+1] += a*prop[l-1] + b*prop[l] = 0 + b*troy(1), hence we need to -b*troy(1) to not double count it
    ...
    propagation[2] += (b*propagation[1]) % MODNUM;
    for (int i=3; i <= n; i++) {
        propagation[i] += (a*propagation[i-2] % MODNUM + b*propagation[i-1] % MODNUM) % MODNUM;
    }
